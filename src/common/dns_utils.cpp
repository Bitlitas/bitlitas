...
#define BITLITAS_DEFAULT_LOG_CATEGORY "net.dns"
...
/**
 * @brief gets a bitlitas address from the TXT record of a DNS entry
 *
 * gets the bitlitas address from the TXT record of the DNS entry associated
 ...
 *
 * @return a bitlitas address (as a string) or an empty string
 */
std::vector<std::string> addresses_from_url(const std::string& url, bool& dnssec_valid)
{
  ...
  // for each txt record, try to find a bitlitas address in it.
...
bool load_txt_records_from_dns(std::vector<std::string> &good_records, const std::vector<std::string> &dns_urls)
{
  ...
  {
    LOG_PRINT_L0("WARNING: no two valid BitlitasPulse DNS checkpoint records were received");
    return false;
  }
  ...
  {
    LOG_PRINT_L0("WARNING: no two BitlitasPulse DNS checkpoint records matched");
    return false;
  }
