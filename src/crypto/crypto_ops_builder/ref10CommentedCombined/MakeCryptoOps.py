...
#written by shen-noether bitlitas research labs
...
if a == "m":
    ...
    fe = glob.glob("fe*.c")
    for g in fe:
        os.system("cp "+g+" "+g.replace("fe", "fe.bitlitas."))
    qhasmToC("fe_pow22523.c", "pow22523.h", "fe.bitlitas._pow22523.c")
    qhasmToC("fe_invert.c", "pow225521.h", "fe.bitlitas._invert.c")
    os.system("rm fe.bitlitas._isnonzero.c") #since it's modified, it's in xmrSpecificOld
    os.system("cat fe.bitlitas.*.c | grep -v '^#include' > fe.bitlitas.c")

    #sc things
    print("\nmaking sc.c")
    print(sc_comments)
    #so you don't get multiple "loads"
    os.system("tail -n +24 sc_reduce.c > sc.bitlitas._reduce.c") #also good on linux
    os.system("tail -n +24 sc_muladd.c > sc.bitlitas._muladd.c")
    os.system("tail -n +31 sc_sub.xmr.c > sc.bitlitas._sub.xmr.c") #careful with the tails if you change these files!
    os.system("cat sc.bitlitas.*.c | grep -v '^#include' > sc.bitlitas.c")

    #ge stuff
    print("making ge.c")
    ge = glob.glob("ge*.c")
    for g in ge:
        os.system("cp "+g+" "+g.replace("ge", "ge.bitlitas."))
    print(ge_comments)
    #need to substitute the below lines for their .h files in the appropriate places
    qhasmToC("ge_add.c", "ge_add.h", "ge.bitlitas._add.c")
    qhasmToC("ge_madd.c", "ge_madd.h", "ge.bitlitas._madd.c")
    qhasmToC("ge_sub.c", "ge_sub.h", "ge.bitlitas._sub.c")
    qhasmToC("ge_msub.c", "ge_msub.h", "ge.bitlitas._msub.c")
    qhasmToC("ge_p2_dbl.c", "ge_p2_dbl.h", "ge.bitlitas._p2_dbl.c")
    qhasmToC("ge_frombytes.c", "d.h", "ge.bitlitas._frombytes.c")
    qhasmToC("ge.bitlitas._frombytes.c", "sqrtm1.h", "ge.bitlitas._frombytes.c")
    qhasmToC("ge_p3_to_cached.c", "d2.h", "ge.bitlitas._p3_to_cached.c")
    ...
    qhasmToC("ge_double_scalarmult.c", "base2.h", "ge.bitlitas._double_scalarmult.c")
    qhasmToC("ge_scalarmult_base.c", "base.h", "ge.bitlitas._scalarmult_base.c")
    #qhasmToC("ge.bitlitas._scalarmult_base.c", "base.h", "ge.bitlitas._scalarmult_base.c")
    os.system("sed -i 's/ cmov/ ge_precomp_cmov/g' ge.bitlitas._scalarmult_base.c")
    os.system("cat ge.bitlitas.*.c | grep -v '^#include' > ge.bitlitas.c")
    ...
    #comments
    with open("fe.bitlitas.comments", "w") as text_file:
            text_file.write(fe_comments)
    with open("ge.bitlitas.comments", "w") as text_file:
            text_file.write(ge_comments)
    with open("sc.bitlitas.comments", "w") as text_file:
            text_file.write(sc_comments)
    with open("xmr.bitlitas.comments", "w") as text_file:
            text_file.write(xmr_comments)
    with open("xmr.bitlitas.predeclarations", "w") as text_file:
            text_file.write(predeclarations)


    #license
    with open("bitlitas.license", "w") as text_file:
            text_file.write(license)

    #crypto-ops.c includes
    with open("crypto-ops.bitlitas.includes", "w") as text_file:
        text_file.write(crypto_ops_includes)

    #note you may have duplicates of load_3, load_4 and possibly some other functions ... 
    os.system("cat bitlitas.license crypto-ops.bitlitas.includes xmr.bitlitas.predeclarations fe.bitlitas.comments fe.bitlitas.c sc.bitlitas.comments sc.bitlitas.c ge.bitlitas.comments ge.bitlitas.c xmr.bitlitas.comments xmrSpecificOld.c > crypto-ops.c")

    #bitlitas specific header files
    ...
    #cleaning up 
    os.system("rm *bitlitas*")

    #bitlitas specific c files
if a == "c":
