...
#define BITLITAS_DEFAULT_LOG_CATEGORY "blockchain"
...
bool Blockchain::init(BlockchainDB* db, const bool testnet, bool offline, const cryptonote::test_options *test_options)
{
  ...
  if (!fakechain) {m_db->set_batch_transactions(true);}
...
void Blockchain::check_against_checkpoints(const checkpoints& points, bool enforce)
{
  ...
  for (const auto& pt : pts)
  {
    ...
    if (!points.check_block(pt.first, m_db->get_block_hash_from_height(pt.first)))
    {
      ...
      {
        LOG_ERROR("WARNING: local blockchain failed to pass a BitlitasPulse checkpoint, and you could be on a fork. You should either sync up from scratch, OR download a fresh blockchain bootstrap, OR enable checkpoint enforcing with the --enforce-dns-checkpointing command-line option");
      }
    }
  }
  if (stop_batch)
...
#if defined(PER_BLOCK_CHECKPOINT)
static const char expected_block_hashes_hash[] = "e0c8b9a457bad620eb0eb0e533475782aa06c943825bdee17aabdab004f2d597";
