...
#define BITLITAS_DEFAULT_LOG_CATEGORY "cn"
...
namespace cryptonote
{
  ...
  static const command_line::arg_descriptor<std::string> arg_check_updates = {
    "check-updates"
  , "Check for new versions of bitlitas: [disabled|notify|download|update]"
  , "notify"
  };
  ...
  bool core::init(const boost::program_options::variables_map& vm, const char *config_subdir, const cryptonote::test_options *test_options)
  {
    ...
    try
    {
      ...
      if (boost::filesystem::exists(old_files / "blockchain.bin"))
      {
        ...
        MWARNING("Bitlitas now uses a new format. You can either remove blockchain.bin to start syncing");
        MWARNING("the blockchain anew, or use bitlitas-blockchain-export and bitlitas-blockchain-import to");
  ...
  bool core::on_idle()
  {
    if(!m_starter_message_showed)
    {
      ...
      if (m_offline)
        main_message = "The daemon is running offline and will not attempt to sync to the Bitlitas network.";
      else
  ...
  {
    static const char software[] = "bitlitas";
#ifdef BUILD_TAG
    ...
    if (tools::vercmp(version.c_str(), BITLITAS_VERSION) <= 0)
      return true;
