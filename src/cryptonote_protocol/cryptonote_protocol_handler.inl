/// @file
/// @author rfree (current maintainer/user in bitlitas.cc project - most of code is from CryptoNote)
...
namespace cryptonote
{
  ...
  int t_cryptonote_protocol_handler<t_core>::handle_notify_new_fluffy_block(int command, NOTIFY_NEW_FLUFFY_BLOCK::request& arg, cryptonote_connection_context& context)
  {
    ...
    if(parse_and_validate_block_from_blob(arg.b.block, new_block))
    {
      ...
      // we do it by index (thanks to a suggestion from bitlitasmooo) because
      ...
      for(auto& tx_blob: arg.b.txs)
      {
        if(parse_and_validate_tx_from_blob(tx_blob, tx))
        {
          ...
          // a possible DOS vector pointed out by @bitlitas-moo where peers keep
          ...
          if(!m_core.pool_has_tx(tx_hash))
          {
            ...
            // according to bitlitas-moo so keep track of these separately ..
            //
          }
        }
        else
  ...
  int t_cryptonote_protocol_handler<t_core>::handle_response_get_objects(int command, NOTIFY_RESPONSE_GET_OBJECTS::request& arg, cryptonote_connection_context& context)
  {
    ...
    //epee::net_utils::network_throttle_manager::get_global_throttle_inreq().logger_handle_net("log/dr-bitlitas/net/req-all.data", sec, get_avg_block_size());
  ...
  bool t_cryptonote_protocol_handler<t_core>::on_connection_synchronized()
  {
    ...
    if(m_synchronized.compare_exchange_strong(val_expected, true))
    {
      MGINFO_YELLOW(ENDL << "**********************************************************************" << ENDL
        << "You are now synchronized with the network. You may now start bitlitas-wallet-cli." << ENDL
        << ENDL
