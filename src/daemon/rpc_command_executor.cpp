...
//    bool ok = windows::stop_service("BitBitlitas Daemon");
//    ok = windows::uninstall_service("BitBitlitas Daemon");
...
bool t_rpc_command_executor::print_status()
{
  ...
  if(daemon_is_alive) {
    tools::success_msg_writer() << "bitlitasd is running";
  }
  else {
    tools::fail_msg_writer() << "bitlitasd is NOT running";
  }
