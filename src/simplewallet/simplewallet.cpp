...
#define OUTPUT_EXPORT_FILE_MAGIC "Bitlitas output export\003"
...
namespace
{
  ...
  const command_line::arg_descriptor<bool> arg_do_not_relay = {"do-not-relay", sw::tr("The newly created transaction will not be relayed to the bitlitas network"), false};
  ...
  std::string oa_prompter(const std::string &url, const std::vector<std::string> &addresses, bool dnssec_valid)
  {
    ...
    prompt << tr("For URL: ") << url
           << ", " << dnssec_str << std::endl
           << tr(" Bitlitas Address = ") << addresses[0]
           << std::endl
           << tr("Is this OK? (Y/n) ")
    ;
...
bool simple_wallet::print_fee_info(const std::vector<std::string> &args/* = std::vector<std::string>()*/)
{
  ...
  message_writer() << (boost::format(tr("Current fee is %s bitlitas per kB")) % print_money(per_kb_fee)).str();
...
bool simple_wallet::export_raw_multisig(const std::vector<std::string> &args)
{
  ...
  try
  {
    ...
    for (auto &ptx: txs.m_ptx)
    {
      ...
      const std::string filename = std::string("raw_multisig_bitlitas_tx_") + epee::string_tools::pod_to_hex(txid);
      if (!filenames.empty())
        filenames += ", ";
      filenames += filename;
  ...
  , m_current_subaddress_account(0)
{
  ...
  m_cmd_binder.set_handler("donate",
                           ...
                           tr("Donate <amount> to the development team (donate.getbitlitas.org)."));
  ...
  m_cmd_binder.set_handler("set",
                           ...
                           tr("Available options:\n "
                                  ...
                                  "ask-password <1|0>\n "
                                  "unit <bitlitas|millinero|micronero|nanonero|piconero>\n "
                                  "  Set the default bitlitas (sub-)unit.\n "
                                  "min-outputs-count [n]\n "
...
#define CHECK_SIMPLE_VARIABLE(name, f, help) do \
  if (args[0] == name) { \
    ...
    CHECK_SIMPLE_VARIABLE("unit", set_unit, tr("bitlitas, millinero, micronero, nanonero, piconero"));
  ...
  const crypto::secret_key& recovery_key, bool recover, bool two_random, const std::string &old_language)
{
  ...
  success_msg_writer() <<
    ...
    "Always use the \"exit\" command when closing bitlitas-wallet-cli to save \n"
...
bool simple_wallet::print_ring_members(const std::vector<tools::wallet2::pending_tx>& ptx_vector, std::ostream& ostr)
{
  ...
  for (size_t n = 0; n < ptx_vector.size(); ++n)
  {
    ...
    for (size_t i = 0; i < tx.vin.size(); ++i)
    {
      ...
      // visualize the distribution, using the code by bitlitasexamples onion-bitlitas-viewer
...
//  "transfer [index=<N1>[,<N2>,...]] [<priority>] [<ring_size>] <address> <amount> [<payment_id>]"
  ...
  try
  {
    ...
    if (m_wallet->multisig())
    {
      bool r = m_wallet->save_multisig_tx(ptx_vector, "multisig_bitlitas_tx");
      if (!r)
      {
      ...
      {
        success_msg_writer(true) << tr("Unsigned transaction(s) successfully written to file: ") << "multisig_bitlitas_tx";
      }
    }
    else if (m_wallet->watch_only())
    {
      bool r = m_wallet->save_tx(ptx_vector, "unsigned_bitlitas_tx");
      if (!r)
      {
        fail_msg_writer() << tr("Failed to write transaction(s) to file");
      }
      ...
      {
        success_msg_writer(true) << tr("Unsigned transaction(s) successfully written to file: ") << "unsigned_bitlitas_tx";
      }
    }
    else
    {
...
bool simple_wallet::donate(const std::vector<std::string> &args_)
{
  ...
  // Hardcode Bitlitas's donation address (see #1447)
  ...
  message_writer() << tr("Donating ") << amount_str << " to Bitlitas (donate.getbitlitas.org/44AFFq5kSiGBoZ4NMDwYtN18obc8AemS33DBLWs3H7otXft3XjrpDtQGv7SqSsaBYBb98uNbr2VBBEt7f2wfn3RVGQBEP3A).";
...
bool simple_wallet::sign_transfer(const std::vector<std::string> &args_)
{
  ...
  try
  {
    bool r = m_wallet->sign_tx("unsigned_bitlitas_tx", "signed_bitlitas_tx", ptx, [&](const tools::wallet2::unsigned_tx_set &tx){ return accept_loaded_tx(tx); }, export_raw);
    if (!r)
    {
  ...
  success_msg_writer(true) << tr("Transaction successfully signed to file ") << "signed_bitlitas_tx" << ", txid " << txids_as_text;
  if (export_raw)
  {
    ...
    for (size_t i = 0; i < ptx.size(); ++i)
    {
      if (i > 0)
        rawfiles_as_text += ", ";
      rawfiles_as_text += "signed_bitlitas_tx_raw" + (ptx.size() == 1 ? "" : ("_" + std::to_string(i)));
    }
...
bool simple_wallet::submit_transfer(const std::vector<std::string> &args_)
{
  ...
  {
    std::vector<tools::wallet2::pending_tx> ptx_vector;
    bool r = m_wallet->load_tx("signed_bitlitas_tx", ptx_vector, [&](const tools::wallet2::signed_tx_set &tx){ return accept_loaded_tx(tx); });
    if (!r)
    {
...
bool simple_wallet::get_tx_proof(const std::vector<std::string> &args)
{
  ...
  try
  {
    ...
    const std::string filename = "bitlitas_tx_proof";
...
bool simple_wallet::get_spend_proof(const std::vector<std::string> &args)
{
  ...
  try
  {
    ...
    const std::string filename = "bitlitas_spend_proof";
    if (epee::file_io_utils::save_string_to_file(filename, sig_str))
      success_msg_writer() << tr("signature file saved to: ") << filename;
    else
      fail_msg_writer() << tr("failed to save signature file");
...
void simple_wallet::commit_or_save(std::vector<tools::wallet2::pending_tx>& ptx_vector, bool do_not_relay)
{
  ...
  while (!ptx_vector.empty())
  {
    ...
    if (do_not_relay)
    {
      ...
      const std::string filename = "raw_bitlitas_tx" + (ptx_vector.size() == 1 ? "" : ("_" + std::to_string(i++)));
...
int main(int argc, char* argv[])
{
  ...
  const auto vm = wallet_args::main(
   argc, argv,
   "bitlitas-wallet-cli [--wallet-file=<file>|--generate-new-wallet=<file>] [<COMMAND>]",
    sw::tr("This is the command line bitlitas wallet. It needs to connect to a bitlitas\ndaemon to work correctly."),
    desc_params,
    positional_options,
    [](const std::string &s, bool emphasis){ tools::scoped_message_writer(emphasis ? epee::console_color_white : epee::console_color_default, true) << s; },
    "bitlitas-wallet-cli.log"
  );
