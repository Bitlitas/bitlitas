...
#define BITLITAS_DEFAULT_LOG_CATEGORY "wallet.wallet2"
...
#define UNSIGNED_TX_PREFIX "Bitlitas unsigned tx set\004"
#define SIGNED_TX_PREFIX "Bitlitas signed tx set\004"
#define MULTISIG_UNSIGNED_TX_PREFIX "Bitlitas multisig unsigned tx set\001"

#define RECENT_OUTPUT_RATIO (0.5) // 50% of outputs are from the recent zone
#define RECENT_OUTPUT_ZONE ((time_t)(1.8 * 86400)) // last 1.8 day makes up the recent zone (taken from bitlitaslink.pdf, Miller et al)
...
#define KEY_IMAGE_EXPORT_FILE_MAGIC "Bitlitas key image export\002"

#define MULTISIG_EXPORT_FILE_MAGIC "Bitlitas multisig export\001"
...
void wallet2::refresh(uint64_t start_height, uint64_t & blocks_fetched, bool& received_money)
{
  if(m_light_wallet) {
    ...
    // MyBitlitas get_address_info needs to be called occasionally to trigger wallet sync.
    // This call is not really needed for other purposes and can be removed if mybitlitas changes their backend.
...
void wallet2::commit_tx(pending_tx& ptx)
{
  ...
  if(m_light_wallet) 
  {
    ...
    // MyBitlitas and OpenBitlitas use different status strings
...
void wallet2::light_wallet_get_outs(std::vector<std::vector<tools::wallet2::get_outs_entry>> &outs, const std::vector<size_t> &selected_transfers, size_t fake_outputs_count) {
  ...
  // Amounts to ask for
  // MyBitlitas api handle amounts and fees as strings
...
bool wallet2::light_wallet_login(bool &new_address)
{
  ...
  m_daemon_rpc_mutex.unlock();
  // MyBitlitas doesn't send any status message. OpenBitlitas does. 
...
void wallet2::light_wallet_get_address_txs()
{
  ...
  //OpenBitlitas sends status=success, Mybitlitas doesn't. 
  ...
  // MyBitlitas doesnt send unlocked balance
...
bool wallet2::light_wallet_key_image_is_ours(const crypto::key_image& key_image, const crypto::public_key& tx_public_key, uint64_t out_index)
{
  ...
  // Subaddresses aren't supported in mybitlitas/openbitlitas yet. Roll out the original scheme:
  //   compute D = a*R
...
std::string wallet2::make_uri(const std::string &address, const std::string &payment_id, uint64_t amount, const std::string &tx_description, const std::string &recipient_name, std::string &error)
{
  ...
  std::string uri = "bitlitas:" + address;
...
{
  if (uri.substr(0, 7) != "bitlitas:")
  {
    error = std::string("URI has wrong scheme (expected \"bitlitas:\"): ") + uri;
    return false;
  }
