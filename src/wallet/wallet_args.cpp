...
namespace wallet_args
{
  ...
  
  {
    ...
    i18n_set_language("translations", "bitlitas", lang);
    ...
    bool r = command_line::handle_error_helper(desc_all, [&]()
    {
      ...
      {
        Print(print) << "Bitlitas '" << BITLITAS_RELEASE_NAME << "' (v" << BITLITAS_VERSION_FULL << ")" << ENDL;
        Print(print) << wallet_args::tr("This is the command line bitlitas wallet. It needs to connect to a bitlitas\n"
      ...
      {
        Print(print) << "Bitlitas '" << BITLITAS_RELEASE_NAME << "' (v" << BITLITAS_VERSION_FULL << ")";
        return false;
      }
    ...
    if (!command_line::is_arg_defaulted(vm, arg_log_level))
      ...
      MINFO("Setting log levels = " << getenv("BITLITAS_LOGS"));
