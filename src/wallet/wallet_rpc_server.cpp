...
#define BITLITAS_DEFAULT_LOG_CATEGORY "wallet.rpc"
...
namespace
{
  ...
  constexpr const char default_rpc_username[] = "bitlitas";
...
#define MKDIR(path, mode)    mkdir(path, mode)
    ...
    else // auth enabled
    {
      if (!rpc_config->login)
      {
        ...
        std::string temp = "bitlitas-wallet-rpc." + bind_port + ".login";
    ...
    for (auto it = destinations.begin(); it != destinations.end(); it++)
    {
      ...
      if(!get_account_address_from_str_or_url(info, m_wallet->testnet(), it->address,
        [&er](const std::string &url, const std::vector<std::string> &addresses, bool dnssec_valid)->std::string {
          ...
          if (addresses.empty())
          {
            er.message = std::string("No Bitlitas address found at ") + url;
            return {};
          }
          return addresses[0];
        }))
...
int main(int argc, char** argv) {
  ...
  const auto vm = wallet_args::main(
    argc, argv,
    "bitlitas-wallet-rpc [--wallet-file=<file>|--generate-from-json=<file>|--wallet-dir=<directory>] [--rpc-bind-port=<port>]",
    tools::wallet_rpc_server::tr("This is the RPC bitlitas wallet. It needs to connect to a bitlitas\ndaemon to work correctly."),
    desc_params,
    po::positional_options_description(),
    [](const std::string &s, bool emphasis){ epee::set_console_color(emphasis ? epee::console_color_white : epee::console_color_default, true); std::cout << s << std::endl; if (emphasis) epee::reset_console_color(); },
    "bitlitas-wallet-rpc.log",
    true
  );
  if (!vm)
  {
