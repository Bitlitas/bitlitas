...
Core tests take longer than any other Bitlitas tests, due to the high amount of computational work involved in validating core components.
...
To run only Bitlitas's core tests (after building):
...
To run only Bitlitas's crypto tests (after building):
...
To run only Bitlitas's hash tests (after building):
...
To run only Bitlitas's performance tests (after building):
...
To run only Bitlitas's unit tests (after building):
