...
std::string WALLETS_ROOT_DIR = "/var/bitlitas/testnet_pvt";
...
struct Utils
{
    ...
    static void print_transaction(Bitlitas::TransactionInfo * t)
    {

        std::cout << "d: "
                  << (t->direction() == Bitlitas::TransactionInfo::Direction_In ? "in" : "out")
                  << ", pe: " << (t->isPending() ? "true" : "false")
                  << ", bh: " << t->blockHeight()
                  << ", a: " << Bitlitas::Wallet::displayAmount(t->amount())
                  << ", f: " << Bitlitas::Wallet::displayAmount(t->fee())
                  << ", h: " << t->hash()
                  << ", pid: " << t->paymentId()
                  << std::endl;
    }
    ...
    {
        Bitlitas::WalletManager *wmgr = Bitlitas::WalletManagerFactory::getWalletManager();
        Bitlitas::Wallet * w = wmgr->openWallet(filename, password, true);
...
{
    Bitlitas::WalletManager * wmgr;


    WalletManagerTest()
    {
        std::cout << __FUNCTION__ << std::endl;
        wmgr = Bitlitas::WalletManagerFactory::getWalletManager();
        // Bitlitas::WalletManagerFactory::setLogLevel(Bitlitas::WalletManagerFactory::LogLevel_4);
...
TEST_F(WalletManagerTest, WalletManagerCreatesWallet)
{
    ...
    Bitlitas::Wallet * wallet = wmgr->createWallet(WALLET_NAME, WALLET_PASS, WALLET_LANG);
    ASSERT_TRUE(wallet->status() == Bitlitas::Wallet::Status_Ok);
...
TEST_F(WalletManagerTest, WalletManagerOpensWallet)
{
    ...
    Bitlitas::Wallet * wallet1 = wmgr->createWallet(WALLET_NAME, WALLET_PASS, WALLET_LANG);
    std::string seed1 = wallet1->seed();
    ASSERT_TRUE(wmgr->closeWallet(wallet1));
    Bitlitas::Wallet * wallet2 = wmgr->openWallet(WALLET_NAME, WALLET_PASS);
    ASSERT_TRUE(wallet2->status() == Bitlitas::Wallet::Status_Ok);
...
{
    LOG_PRINT_L3("max amount: " << Bitlitas::Wallet::displayAmount(
                     Bitlitas::Wallet::maximumAllowedAmount()));
...
{
    uint64_t amount = Bitlitas::Wallet::amountFromString("18446740");
    ASSERT_TRUE(amount > 0);
    amount = Bitlitas::Wallet::amountFromString("11000000000000");
    ASSERT_FALSE(amount > 0);
    amount = Bitlitas::Wallet::amountFromString("0.0");
    ASSERT_FALSE(amount > 0);
    amount = Bitlitas::Wallet::amountFromString("10.1");
    ASSERT_TRUE(amount > 0);

}

void open_wallet_helper(Bitlitas::WalletManager *wmgr, Bitlitas::Wallet **wallet, const std::string &pass, boost::mutex *mutex)
{
    if (mutex)
        mutex->lock();
...
//    Bitlitas::Wallet * wallet1 = wmgr->createWallet(WALLET_NAME, wallet_pass, WALLET_LANG, true);
//    std::string seed1 = wallet1->seed();
//    ASSERT_TRUE(wmgr->closeWallet(wallet1));

//    Bitlitas::Wallet *wallet2 = nullptr;
//    Bitlitas::Wallet *wallet3 = nullptr;
...
//    ASSERT_TRUE(wallet3->status() == Bitlitas::Wallet::Status_Ok);
...
TEST_F(WalletManagerTest, WalletManagerMovesWallet)
{
    ...
    Bitlitas::Wallet * wallet2 = wmgr->openWallet(WALLET_NAME_MOVED, WALLET_PASS);
...
TEST_F(WalletManagerTest, WalletManagerChangesPassword)
{
    ...
    ASSERT_TRUE(wmgr->closeWallet(wallet1));
    Bitlitas::Wallet * wallet2 = wmgr->openWallet(WALLET_NAME, WALLET_PASS2);
    ASSERT_TRUE(wallet2->status() == Bitlitas::Wallet::Status_Ok);
    ASSERT_TRUE(wallet2->seed() == seed1);
    ASSERT_TRUE(wmgr->closeWallet(wallet2));
    Bitlitas::Wallet * wallet3 = wmgr->openWallet(WALLET_NAME, WALLET_PASS);
    ASSERT_FALSE(wallet3->status() == Bitlitas::Wallet::Status_Ok);
}
...
TEST_F(WalletManagerTest, WalletManagerRecoversWallet)
{
    ...
    ASSERT_TRUE(wmgr->closeWallet(wallet1));
    Utils::deleteWallet(WALLET_NAME);
    Bitlitas::Wallet * wallet2 = wmgr->recoveryWallet(WALLET_NAME, seed1);
    ASSERT_TRUE(wallet2->status() == Bitlitas::Wallet::Status_Ok);
    ASSERT_TRUE(wallet2->seed() == seed1);
...
TEST_F(WalletManagerTest, WalletManagerStoresWallet1)
{
    ...
    ASSERT_TRUE(wmgr->closeWallet(wallet1));
    Bitlitas::Wallet * wallet2 = wmgr->openWallet(WALLET_NAME_COPY, WALLET_PASS);
    ASSERT_TRUE(wallet2->status() == Bitlitas::Wallet::Status_Ok);
    ASSERT_TRUE(wallet2->seed() == seed1);
    ASSERT_TRUE(wallet2->mainAddress() == address1);
    ASSERT_TRUE(wmgr->closeWallet(wallet2));
...
TEST_F(WalletManagerTest, WalletManagerStoresWallet2)
{
    ...
    ASSERT_TRUE(wallet1->status() == Bitlitas::Wallet::Status_Ok);
...
TEST_F(WalletManagerTest, WalletManagerStoresWallet3)
{
    ...
    ASSERT_FALSE(wallet1->status() == Bitlitas::Wallet::Status_Ok);
...
{
    std::string payment_id = Bitlitas::Wallet::genPaymentId();
...
TEST_F(WalletTest1, WalletGeneratesIntegratedAddress)
{
    ...
    Bitlitas::Wallet * wallet1 = wmgr->openWallet(CURRENT_SRC_WALLET, TESTNET_WALLET_PASS, true);
...
TEST_F(WalletTest1, WalletShowsBalance)
{
    ...
    ASSERT_TRUE(wmgr->closeWallet(wallet1));
    Bitlitas::Wallet * wallet2 = wmgr->openWallet(CURRENT_SRC_WALLET, TESTNET_WALLET_PASS, true);
...
{
    std::string strAmount = Bitlitas::Wallet::displayAmount(AMOUNT_5XMR);
    ASSERT_TRUE(AMOUNT_5XMR == Bitlitas::Wallet::amountFromString(strAmount));

    ASSERT_TRUE(AMOUNT_5XMR == Bitlitas::Wallet::amountFromDouble(5.0));
    ASSERT_TRUE(AMOUNT_10XMR == Bitlitas::Wallet::amountFromDouble(10.0));
    ASSERT_TRUE(AMOUNT_1XMR == Bitlitas::Wallet::amountFromDouble(1.0));
...

{
    ...
    ASSERT_TRUE(wallet1->status() == Bitlitas::PendingTransaction::Status_Ok);

    std::string recepient_address = Utils::get_wallet_address(CURRENT_DST_WALLET, TESTNET_WALLET_PASS);
    const int MIXIN_COUNT = 4;


    Bitlitas::PendingTransaction * transaction = wallet1->createTransaction(recepient_address,
                                                                             PAYMENT_ID_EMPTY,
                                                                             AMOUNT_10XMR,
                                                                             MIXIN_COUNT,
                                                                             Bitlitas::PendingTransaction::Priority_Medium,
                                                                             0,
                                                                             std::set<uint32_t>{});
    ASSERT_TRUE(transaction->status() == Bitlitas::PendingTransaction::Status_Ok);
    wallet1->refresh();
...
TEST_F(WalletTest1, WalletTransactionWithMixin)
{
    ...
    for (auto mixin : mixins) {
        ...
        Bitlitas::PendingTransaction * transaction = wallet1->createTransaction(
                    recepient_address, payment_id, AMOUNT_5XMR, mixin, Bitlitas::PendingTransaction::Priority_Medium, 0, std::set<uint32_t>{});

        std::cerr << "Transaction status: " << transaction->status() << std::endl;
        std::cerr << "Transaction fee: " << Bitlitas::Wallet::displayAmount(transaction->fee()) << std::endl;
...
TEST_F(WalletTest1, WalletTransactionWithPriority)
{
    ...
    std::vector<Bitlitas::PendingTransaction::Priority> priorities =  {
         Bitlitas::PendingTransaction::Priority_Low,
         Bitlitas::PendingTransaction::Priority_Medium,
         Bitlitas::PendingTransaction::Priority_High
    };
...
TEST_F(WalletTest1, WalletHistory)
{
    ...
    ASSERT_TRUE(wallet1->init(TESTNET_DAEMON_ADDRESS, 0));
    ASSERT_TRUE(wallet1->refresh());
    Bitlitas::TransactionHistory * history = wallet1->history();
    history->refresh();
...
{
    return;
    Bitlitas::Wallet * wallet_src = wmgr->openWallet(CURRENT_SRC_WALLET, TESTNET_WALLET_PASS, true);
    // make sure testnet daemon is running
    ASSERT_TRUE(wallet_src->init(TESTNET_DAEMON_ADDRESS, 0));
    ASSERT_TRUE(wallet_src->refresh());
    Bitlitas::TransactionHistory * history = wallet_src->history();
    history->refresh();
    ASSERT_TRUE(history->count() > 0);
    ...
    Bitlitas::PendingTransaction * tx = wallet_src->createTransaction(wallet4_addr,
                                                                       PAYMENT_ID_EMPTY,
                                                                       AMOUNT_10XMR * 5, 1, Bitlitas::PendingTransaction::Priority_Medium, 0, std::set<uint32_t>{});

    ASSERT_TRUE(tx->status() == Bitlitas::PendingTransaction::Status_Ok);
...
TEST_F(WalletTest1, WalletTransactionWithPaymentId)
{
    ...
    Bitlitas::PendingTransaction * tx = wallet_src->createTransaction(wallet4_addr,
                                                                       payment_id,
                                                                       AMOUNT_1XMR, 1, Bitlitas::PendingTransaction::Priority_Medium, 0, std::set<uint32_t>{});
...
struct MyWalletListener : public Bitlitas::WalletListener
{

    Bitlitas::Wallet * wallet;
    uint64_t total_tx;
    uint64_t total_rx;
    boost::mutex  mutex;
    ...
    MyWalletListener(Bitlitas::Wallet * wallet)
...
TEST_F(WalletTest2, WalletCallbackSent)
{
    ...
    Bitlitas::Wallet * wallet_dst = wmgr->openWallet(CURRENT_DST_WALLET, TESTNET_WALLET_PASS, true);

    uint64_t amount = AMOUNT_1XMR * 5;
    std::cout << "** Sending " << Bitlitas::Wallet::displayAmount(amount) << " to " << wallet_dst->mainAddress();


    Bitlitas::PendingTransaction * tx = wallet_src->createTransaction(wallet_dst->mainAddress(),
                                                                       PAYMENT_ID_EMPTY,
                                                                       amount, 1, Bitlitas::PendingTransaction::Priority_Medium, 0, std::set<uint32_t>{});
    std::cout << "** Committing transaction: " << Bitlitas::Wallet::displayAmount(tx->amount())
              << " with fee: " << Bitlitas::Wallet::displayAmount(tx->fee());
...
TEST_F(WalletTest2, WalletCallbackNewBlock)
{
    ...
    Bitlitas::Wallet * wallet_src = wmgr->openWallet(TESTNET_WALLET5_NAME, TESTNET_WALLET_PASS, true);
    // make sure testnet daemon is running
    ASSERT_TRUE(wallet_src->init(TESTNET_DAEMON_ADDRESS, 0));
    ASSERT_TRUE(wallet_src->refresh());
...
TEST_F(WalletManagerMainnetTest, CreateOpenAndRefreshWalletMainNetSync)
{
    ...
    Bitlitas::Wallet * wallet = wmgr->createWallet(WALLET_NAME_MAINNET, "", WALLET_LANG);
...
int main(int argc, char** argv)
{
    ...
    Bitlitas::WalletManagerFactory::setLogLevel(Bitlitas::WalletManagerFactory::LogLevel_Max);
