...
#define UNSIGNED_TX_PREFIX "Bitlitas unsigned tx set\003"
TEST(Serialization, portability_unsigned_tx)
{
  const boost::filesystem::path filename = unit_test::data_dir / "unsigned_bitlitas_tx";
  std::string s;
  const bool testnet = true;
...
#define SIGNED_TX_PREFIX "Bitlitas signed tx set\003"
TEST(Serialization, portability_signed_tx)
{
  const boost::filesystem::path filename = unit_test::data_dir / "signed_bitlitas_tx";
  const bool testnet = true;
  std::string s;
  bool r = epee::file_io_utils::load_file_to_string(filename.string(), s);
  ASSERT_TRUE(r);
